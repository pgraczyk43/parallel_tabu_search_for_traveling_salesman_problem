...
# Global CUDA Usage Setting
USE_CUDA = False  # Set to False to disable CUDA

# Check CUDA availability only if CUDA is enabled
if USE_CUDA:
...
def distributed_tabu_search(file_path, start_id, total_iterations=100, workers=4):
    iterations_per_worker = total_iterations // workers
    tasks = [tabu_search_worker.remote(file_path, start_id, iterations_per_worker, seed) for seed in range(workers)]
    results = ray.get(tasks)
    best_solution, best_cost, cost_history = min(results, key=lambda x: x[1])
    return best_solution, best_cost, cost_history
...
if __name__ == "__main__":
    ...
    best_solution, best_cost, cost_history = distributed_tabu_search(file_path, start_id, total_iterations, workers)
    end_time = time.time()
